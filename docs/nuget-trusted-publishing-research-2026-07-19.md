...
> 后续决策注释（2026-07-19）：首发成功后，仓库所有者决定将“受控 tag push”作为 NuGet 发布的最终人工授权，不为 `nuget-production` 增加 required reviewer。本文的查询日期事实保持不变；下文涉及 required reviewer 的原建议已按最终决策修正。严格 SemVer/project version、双平台 release gate、OIDC、`nuget-production`、受控 tag 创建/更新权限，以及发布前人工核对 peeled commit、`origin/master` 和同一 SHA CI，仍是发布边界。当前 workflow 尚未强制执行后三项核对，实际 tag ruleset 也尚未由仓库证据证明。
...
当前 workflow 可以迁移，而且不需要改变验证、打包或 environment 边界的总体结构。最小改造是：
...
5. 保留 `nuget-production` 和 deployment branch/tag protection；受控 tag push 作为最终发布授权，不增加 required reviewer，并明确处理已发布版本的 rerun。

实施状态（首发后更新）：本仓库已按上述方案使用 `NuGet/login` OIDC 登录，并从
`nuget-production` environment variable `NUGET_USERNAME` 读取 NuGet.org profile
username。发布 run `29687047745` 已证明 Trusted Publishing policy、workflow、environment、
OIDC login、主包和 symbols push 链路可用。GitHub environment 当前只配置 tag `v*.*.*`
的 deployment branch policy，没有 required reviewer；实际 tag ruleset 仍未由仓库证据证明。
...
按后续最终决策，应在 GitHub 仓库设置中确认：

- `nuget-production` 不增加 required reviewer；受控 tag push 是最终人工发布授权，门禁成功后允许自动发布。
...
- 建议保留失败即停，并写明恢复手册：若主包已存在，先验证 NuGet.org repository signature，再比较 package ID/version/repository commit，以及除 NuGet.org 新增 `.signature.p7s` 外的逐条目内容。NuGet.org 会增加 repository signature，不能要求远端 `.nupkg` 整包 hash 与原 run artifact 相等。内容一致时只重推缺失的 `.snupkg`，不一致时停止并调查；不要盲目 rerun 整个 publish job。
...
2. **核对 GitHub environment 与 tag 权限**：确认不依赖 required reviewer，核对 deployment tag restrictions 和受保护 tag 创建/更新权限。先完成保护再授予 OIDC。
...
5. **做受控首发**：将受控 tag push 作为最终人工授权，随后观察两端门禁、login、主包和 symbols 各阶段；不要输出 token，也不要启用会打印环境变量的 debug 脚本。
...
调研阶段仅凭仓库文件无法证明 NuGet.org policy 或 GitHub environment 的实时配置，因此上线前要求在两个服务 UI 中确认 owner/repository、NuGet username、environment protection、deployment tag rules 和 policy active 状态。

该调研本身没有调用 NuGet token endpoint 或发布测试包；这个调研限制后来已由成功的首发 run `29687047745` 覆盖。首发同时证明 `nuget-production` 没有人工审批停止点。`NuGet/login v1.2.0` 的固定 SHA 与 `node24` action metadata 也已在后续加固中重新核验。仍未解决的是实际 tag 创建/更新 ruleset 的证明。
